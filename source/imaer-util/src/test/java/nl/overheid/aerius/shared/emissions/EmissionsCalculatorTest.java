...
class EmissionsCalculatorTest {
  ...
  @Test
  void testVisitFarmAnimalHousing() throws AeriusException {
    final Map<Substance, Double> calculatedEmissions = Map.of(Substance.NH3, 324.0);
    final FarmAnimalHousingEmissionSource emissionSource = mock(FarmAnimalHousingEmissionSource.class);
    when(farmAnimalHousingCalculator.calculateEmissions(emissionSource)).thenReturn(calculatedEmissions);

    final Map<Substance, Double> result = emissionsCalculator.visit(emissionSource, null);

    assertEquals(calculatedEmissions, result,
        "Calculated emission values from FarmAnimalHousingEmissionSource should be returned for farm animal housing source");
  }
